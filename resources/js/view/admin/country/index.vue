<template>
    <div class="page-wrapper">
        <div class="content container-fluid">
                            ...
                            <div class="card-header pt-0">
                                <div class="row justify-content-between">
                                    ...
                                    <div class="col-5 row justify-content-end">
                                        <router-link
                                            :to="{name: 'createCountry', params: {lang: locale || 'ar'}}"
                                ...
                                <table class="table mb-0">
                                    ...
                                    <tbody>
                                        <tr v-for="(item,index) in countries" v-if="countries" :key="item.id">
                                            ...
                                            <td>
                                                <router-link :to="{name: 'editCountry', params: {lang: locale || 'ar',id:item.id}}" class="btn btn-sm btn-success me-2">
            ...
            <!-- /Table -->
            <!-- start Pagination -->
            <Pagination :data="countriesPaginate" @pagination-change-page="getCountries">
                <template #prev-nav>
                    <span>&lt; Previous</span>
                </template>
                <template #next-nav>
...
export default {
    ...
    setup(){
        ...
        // get packages
        let countries = ref([]);
        let countriesPaginate = ref({});
        let loading = ref(false);
        const search = ref('');

        let getCountries = (page = 1) => {
            loading.value = true;

            adminApi.get(`/v1/dashboard/country?page=${page}&search=${search.value}`)
                .then((res) => {
                    let l = res.data.data;
                    countriesPaginate.value = l.countries;
                    countries.value = l.countries.data;
                })
                .catch((err) => {
         ...
         function deleteCountry(id,index){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    adminApi.delete(`/v1/dashboard/country/${id}`)
                        .then((res) => {
                            countries.value.splice(index,index + 1);
        ...
        return {countries,loading,getCountries,search,deleteCountry,countriesPaginate};
