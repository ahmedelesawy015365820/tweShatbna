...
export default {
    ...
    setup(props){
        ...
        let getDATA = () => {
            ...
            adminApi.get(`/v1/dashboard/state/${id.value}/edit`)
                .then((res) => {
                    let l = res.data.data;
                    countries.value = l.country;

                    addState.data.en.name = l.state.translations[1].name;
                    addState.data.ar.name = l.state.translations[0].name;
                    addState.data.country_id = l.state.country_id;
                    addState.data.status = l.state.status;
        ...
        return {loading,...toRefs(addState),v$,countries,id};
    },
    methods: {
        storeState(){
            this.v$.$validate();
            ...
            if(!this.v$.$error){
                ...
                adminApi.put(`/v1/dashboard/state/${this.id}`,this.data)
                    .then((res) => {
