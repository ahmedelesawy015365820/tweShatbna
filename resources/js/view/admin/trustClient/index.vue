...
<!--                                    </div>-->
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tbody>
                                    <tr v-for="(item,index) in clients" v-if="clients" :key="item.id">
                                        <td>{{index + 1}}</td>
                                        ...
                                        <td>
                                            <router-link
                                                :to="{name: 'showClient',params: {lang: locale || 'ar',id:item.id}}"
                                                class="btn btn-sm btn-info me-2"
                                                v-if="parseInt(item.client.send)"
                                                >
            ...
            <!-- /Table -->
            <!-- start Pagination -->
            <Pagination :data="clientPaginate" @pagination-change-page="getClient">
                <template #prev-nav>
                    <span>&lt; Previous</span>
                </template>
                <template #next-nav>
...
export default {
    ...
    setup(){
        ...
        let getClient = (page = 1) => {
            ...
            adminApi.get(`/v1/dashboard/trustClient?page=${page}&search=${search.value}`)
                .then((res) => {
                    let l = res.data.data;
                    clientPaginate.value = l.clients;
        ...
        return {clientPaginate,loading,getClient,search,clients};
