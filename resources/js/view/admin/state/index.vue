<template>
    <div class="page-wrapper">
        <div class="content container-fluid">
                        ...
                        <div class="card-body">
                            <div class="card-header pt-0">
                                <div class="row justify-content-between">
                                    ...
                                    <div class="col-5 row justify-content-end">
                                        <router-link
                                            :to="{name: 'createState', params: {lang: locale || 'ar'}}"
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tbody>
                                        <tr v-for="(item,index) in states" v-if="states" :key="item.id">
                                            <td>{{index + 1}}</td>
                                            <td>{{item.name}}</td>
                                            <td>{{item.country.name}}</td>
                                            <td><span :class="[parseInt(item.status) ? 'text-success': 'text-danger']">{{parseInt(item.status) ? 'Active': 'Inactive'}}</span></td>
                                            <td>
                                                <router-link :to="{name: 'editState', params: {lang: locale || 'ar',id:item.id}}" class="btn btn-sm btn-success me-2">
            ...
            <!-- /Table -->
            <!-- start Pagination -->
            <Pagination :data="statesPaginate" @pagination-change-page="getStates">
                <template #prev-nav>
                    <span>&lt; Previous</span>
                </template>
                <template #next-nav>
...
export default {
    ...
    setup(){
        ...
        // get packages
        let states = ref([]);
        let statesPaginate = ref({});
        let loading = ref(false);
        const search = ref('');

        let getStates = (page = 1) => {
            loading.value = true;

            adminApi.get(`/v1/dashboard/state?page=${page}&search=${search.value}`)
                .then((res) => {
                    let l = res.data.data;
                    statesPaginate.value = l.states;
                    states.value = l.states.data;
                })
                .catch((err) => {
         ...
         function deleteCountry(id,index){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    adminApi.delete(`/v1/dashboard/state/${id}`)
                        .then((res) => {
                            states.value.splice(index,index + 1);
        ...
        return {states,loading,getStates,search,deleteCountry,statesPaginate};
