                        ...
                        <div class="card-body">
                            <div class="card-header pt-0">
                                <div class="row justify-content-between">
                                    ...
                                    <div class="col-5 row justify-content-end">
                                        <router-link
                                            :to="{name: 'createRole', params: {lang: locale || 'ar'}}"
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tbody>
                                    <tr v-for="(item,index) in roles" v-if="roles" :key="item.id">
                                        <td>{{ index + 1 }}</td>
                                        <td>{{ item.name }}</td>

                                        <td>
                                            <router-link
                                                :to="{name: 'editRole', params: {lang: locale || 'ar',id:item.id}}"
                                                class="btn btn-sm btn-success me-2">
                                                <i class="far fa-edit"></i>
                                            </router-link>

                                            <a href="#" @click="deleteRole(item.id,item.name,index)"
...
export default {
    ...
    setup() {
        ...
        let getRole = (page = 1) => {
            ...
            adminApi.get(`/v1/dashboard/role?page=${page}&search=${search.value}`)
                .then((res) => {
                    let l = res.data.data;
        ...
        function deleteRole(id, roleName, index) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    adminApi.delete(`/v1/dashboard/role/${id}`)
                        .then((res) => {
        ...
        return {roles, loading, getRole, search, deleteRole, rolesPaginate};
