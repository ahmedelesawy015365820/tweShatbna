...
<!--                                    </div>-->
            ...
            <!-- /Table -->
            <!-- start Pagination -->
            <Pagination :data="designPaginate" @pagination-change-page="getDesigns">
                <template #prev-nav>
                    <span>&lt; Previous</span>
                </template>
                <template #next-nav>
...
export default {
    ...
    setup(){
        ...
        // get packages
        let designs = ref([]);
        let designPaginate = ref({});
        let loading = ref(false);
        const search = ref('');

        let getDesigns = (page = 1) => {
            loading.value = true;

            adminApi.get(`/v1/dashboard/trustDesign?page=${page}&search=${search.value}`)
                .then((res) => {
                    let l = res.data.data;
                    designs.value =  l.design.data;
                    designPaginate.value = l.design;
                })
                .catch((err) => {
        ...
        return {designPaginate,loading,getDesigns,search,designs};
